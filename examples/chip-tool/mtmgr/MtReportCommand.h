...
{
public:
    MtReportCommand(MatterManagerCore & mtmgrCore) :
        InteractionModelReports(this), MtModelCommand(mtmgrCore, /* supportsMultipleEndpoints = */ true)
    {}
...
protected:
    ...
    {
        return mTimeout.HasValue() ? chip::System::Clock::Seconds16(mTimeout.Value()) : (MtModelCommand::GetWaitDuration() * 3);
    }

    CHIP_ERROR mError = CHIP_NO_ERROR;
};

class MtReadCommand : public MtReportCommand
{
protected:
    MtReadCommand(MatterManagerCore & mtmgrCore) : MtReportCommand(mtmgrCore) {}
...
class MtReadAttribute : public MtReadCommand
{
public:
    MtReadAttribute(MatterManagerCore & mtmgrCore) : MtReadCommand(mtmgrCore)
    {
#if 0
    ...
    MtReadAttribute(chip::ClusterId clusterId, MatterManagerCore & mtmgrCore) : MtReadCommand(mtmgrCore), mClusterIds(1, clusterId)
    {
#if 0
        AddAttributeIdArgument();
        AddCommonArguments();
    ...
    MtReadAttribute(chip::ClusterId clusterId, chip::AttributeId attributeId, MatterManagerCore & mtmgrCore) :
        MtReadCommand(mtmgrCore), mClusterIds(1, clusterId), mAttributeIds(1, attributeId)
    {
#if 0
    ...
    {
        return MtReadCommand::ReadAttribute(device, endpointIds, mClusterIds, mAttributeIds);
    }
