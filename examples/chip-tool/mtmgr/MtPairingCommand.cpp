...
CHIP_ERROR MtPairingCommand::RunCommand()
{
    mMtmgrCore.CurrentCommissioner().RegisterPairingDelegate(this);
    // Clear the CATs in OperationalCredentialsIssuer
    mMtmgrCore.GetCredIssuerCmds()->SetCredentialIssuerCATValues(kUndefinedCATs);
    ...
    if (mCASEAuthTags.HasValue() && mCASEAuthTags.Value().size() <= kMaxSubjectCATAttributeCount)
    {
        ...
        if (cats.AreValid())
        {
            mMtmgrCore.GetCredIssuerCmds()->SetCredentialIssuerCATValues(cats);
        }
    }
    return RunInternal(mNodeId);
}

CHIP_ERROR MtPairingCommand::RunInternal(NodeId remoteId)
{
...
CommissioningParameters MtPairingCommand::GetCommissioningParameters()
{
    ...
    if (!mSkipICDRegistration.ValueOr(false))
    {
        ...
        {
            mICDCheckInNodeId.SetValue(mMtmgrCore.CurrentCommissioner().GetNodeId());
        }
...
CHIP_ERROR MtPairingCommand::PaseWithCode(NodeId remoteId)
{
    ...
    return mMtmgrCore.CurrentCommissioner().EstablishPASEConnection(remoteId, mOnboardingPayload, discoveryType);
}

CHIP_ERROR MtPairingCommand::PairWithCode(NodeId remoteId)
{
    ...
    return mMtmgrCore.CurrentCommissioner().PairDevice(remoteId, mOnboardingPayload, commissioningParams, discoveryType);
}

CHIP_ERROR MtPairingCommand::Pair(NodeId remoteId, PeerAddress address)
{
    ...
    {
        err = mMtmgrCore.CurrentCommissioner().EstablishPASEConnection(remoteId, params);
    }
    else
    {
        auto commissioningParams = GetCommissioningParameters();
        err                      = mMtmgrCore.CurrentCommissioner().PairDevice(remoteId, params, commissioningParams);
    }
    return err;
}

CHIP_ERROR MtPairingCommand::PairWithMdnsOrBleByIndex(NodeId remoteId, uint16_t index)
{
...
CHIP_ERROR MtPairingCommand::PairWithMdnsOrBleByIndexWithCode(NodeId remoteId, uint16_t index)
{
#if CHIP_DEVICE_LAYER_TARGET_DARWIN
    VerifyOrReturnError(IsInteractive(), CHIP_ERROR_INCORRECT_STATE);
    ...
    if (mPaseOnly.ValueOr(false))
    {
        err = mMtmgrCore.CurrentCommissioner().EstablishPASEConnection(
            remoteId, mOnboardingPayload, DiscoveryType::kDiscoveryNetworkOnly, MakeOptional(resolutionData));
    }
    else
    {
        auto commissioningParams = GetCommissioningParameters();
        err                      = mMtmgrCore.CurrentCommissioner().PairDevice(remoteId, mOnboardingPayload, commissioningParams,
...
CHIP_ERROR MtPairingCommand::PairWithMdns(NodeId remoteId)
{
    ...
    mMtmgrCore.CurrentCommissioner().RegisterDeviceDiscoveryDelegate(this);
    return mMtmgrCore.CurrentCommissioner().DiscoverCommissionableNodes(filter);
}
...
void MtPairingCommand::OnStatusUpdate(DevicePairingDelegate::Status status)
{
    switch (status)
    {
...
void MtPairingCommand::OnPairingComplete(CHIP_ERROR err)
{
...
void MtPairingCommand::OnPairingDeleted(CHIP_ERROR err)
{
    if (err == CHIP_NO_ERROR)
    {
...
void MtPairingCommand::OnCommissioningComplete(NodeId nodeId, CHIP_ERROR err)
{
    if (err == CHIP_NO_ERROR)
    {
    ...
    else
    {
        ...
        if (mDeviceIsICD)
        {
            CHIP_ERROR deleteEntryError = mMtmgrCore.GetICDClientStorage().DeleteEntry(
                ScopedNodeId(mNodeId, mMtmgrCore.CurrentCommissioner().GetFabricIndex()));
...
void MtPairingCommand::OnReadCommissioningInfo(const Controller::ReadCommissioningInfo & info)
{
...
void MtPairingCommand::OnICDRegistrationComplete(NodeId nodeId, uint32_t icdCounter)
{
    ...
    clientInfo.peer_node         = ScopedNodeId(nodeId, mMtmgrCore.CurrentCommissioner().GetFabricIndex());
    clientInfo.monitored_subject = mICDMonitoredSubject.Value();
    clientInfo.start_icd_counter = icdCounter;

    CHIP_ERROR err = mMtmgrCore.GetICDClientStorage().SetKey(clientInfo, mICDSymmetricKey.Value());
    if (err == CHIP_NO_ERROR)
    {
        err = mMtmgrCore.GetICDClientStorage().StoreEntry(clientInfo);
    }

    if (err != CHIP_NO_ERROR)
    {
        mMtmgrCore.GetICDClientStorage().RemoveKey(clientInfo);
...
void MtPairingCommand::OnDiscoveredDevice(const chip::Dnssd::DiscoveredNodeData & nodeData)
{
    ...
    // Stop Mdns discovery.
    auto err = mMtmgrCore.CurrentCommissioner().StopCommissionableDiscovery();
    ...
    mMtmgrCore.CurrentCommissioner().RegisterDeviceDiscoveryDelegate(nullptr);
...
void MtPairingCommand::OnCurrentFabricRemove(void * context, NodeId nodeId, CHIP_ERROR err)
{
    MtPairingCommand * command = reinterpret_cast<MtPairingCommand *>(context);
...
chip::Optional<uint16_t> MtPairingCommand::FailSafeExpiryTimeoutSecs() const
{
...
void MtPairingCommand::OnDeviceAttestationCompleted(
